...
func newReverseProxy(name string, transport http.RoundTripper, log *slog.Logger, checker *health.Checker) *httputil.ReverseProxy {
	return &httputil.ReverseProxy{
		...
		Rewrite: func(pr *httputil.ProxyRequest) {
			...
			// The span middleware.Tracing started is on this same
			// context (WithTimeout/WithValue in proxyHandler preserve
			// it) - inject it as an outgoing traceparent so the
			// upstream can continue the same trace.
